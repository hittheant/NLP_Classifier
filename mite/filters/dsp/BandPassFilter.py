...
class BandPassFilter(AbstractBaseFilter):
    """ A Python implementation of a Butterworth bandpass filter """
    def __init__(self, low_cutoff, high_cutoff, fs, order = 5):
        ...
        -------
        obj
            A BandPassFilter object
        """  
        nyq = 0.5 * fs
        lo_normal_cutoff = low_cutoff / nyq
        hi_normal_cutoff = high_cutoff / nyq
        b, a = butter( order, [ lo_normal_cutoff, hi_normal_cutoff], btype = 'bandpass', analog = False )
...
if __name__ == '__main__':
    ...
    bpf = BandPassFilter(low_cutoff = lowcutoff, high_cutoff = highcutoff, fs = fs, order = order)
    w, h = freqz( bpf._BandPassFilter__butter[0], bpf._BandPassFilter__butter[1], worN = 8000 )
    plt.subplot( 2, 1, 1 )
